...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_04c)
		{
			...
			auto total_age = std::accumulate(v.begin(), v.end(), 0, [](int sum, person p) {return sum + p.age; });// TODO: sum of all ages
		...
		TEST_METHOD(test_05a)
		{
			...
			auto number_of_negative = std::count_if(v.begin(), v.end(), [](int x) {return x < 0; });// TODO: 
		...
		TEST_METHOD(test_05c)
		{
			...
			auto number_in_first_quadrant = std::count_if(v.begin(), v.end(), [](point p) {return p.x > 0 && p.y > 0; });// TODO: 
		...
		TEST_METHOD(test_12)
		{
			...
			auto smallest_difference = std::abs(*std::max_element(atp_points.begin() + 1, atp_points.end()));// TODO: 
